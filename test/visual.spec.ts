...
describe('Visual mode operations', () => {
  ...
  it('enters visual and yanks/deletes/pastes', () => {
    ...
    eng.enterVisual();
    assert.equal(eng.mode.current, MODES.VISUAL);
    eng.cursor.setPosition(1, 0);
    eng.updateSelectionRect();
    eng.yankSelection();
    eng.exitVisual();
    eng.cursor.setPosition(0, 1);
    eng.pasteAtCursor();
    assert.equal(eng.grid.cells[1][0], 1);
    assert.equal(eng.grid.cells[1][1], 2);
    eng.cursor.setPosition(0, 0);
    eng.enterVisual();
    eng.cursor.setPosition(1, 0);
    eng.updateSelectionRect();
    ...
    assert.equal(eng.grid.cells[0][1], null);
  });
  ...
  it('fill, stroke rect, line, flood fill', () => {
    ...
    assert.ok(eng.grid.cells[1][1] != null);
    press(eng, 'v', 'l', 'l', 'R');
    eng.cursor.setPosition(0, 0);
    press(eng, 'v');
    eng.cursor.setPosition(4, 4);
    press(eng, 'L');
  ...
  it('stroke and fill circle selections', () => {
    ...
    assert.ok(eng.grid.cells[0][2] != null);
    assert.ok(eng.grid.cells[2][0] != null);
    assert.equal(eng.grid.cells[2][2], null);

    eng.cursor.setPosition(0, 0);
    press(eng, 'v', 'l', 'l', 'l', 'l', 'j', 'j', 'j', 'j', 'O');
    assert.equal(eng.mode.current, MODES.NORMAL);
    assert.ok(eng.grid.cells[2][2] != null);
  });
  ...
  it('fill selection only fills selected area and returns to normal mode', () => {
    ...
    assert.equal(eng.grid.cells[1][1], 3); // inside selection
    assert.equal(eng.grid.cells[2][2], 3); // inside selection
    assert.equal(eng.grid.cells[3][3], 3); // inside selection

    // Check that cells outside selection are NOT filled
    assert.equal(eng.grid.cells[0][0], null); // outside selection
    assert.equal(eng.grid.cells[0][1], null); // outside selection
    assert.equal(eng.grid.cells[4][4], null); // outside selection
    assert.equal(eng.grid.cells[4][0], null); // outside selection

    // Move cursor and verify it doesn't paint while moving in normal mode
    eng.cursor.setPosition(0, 0);
    eng.move(1, 0); // move right
    assert.equal(eng.grid.cells[0][0], null); // should still be null
    assert.equal(eng.grid.cells[0][1], null); // should still be null

    eng.move(0, 1); // move down
    assert.equal(eng.grid.cells[1][1], 3); // this was already filled, shouldn't change
    assert.equal(eng.grid.cells[1][0], null); // this should still be null
  });
  ...
  it('user scenario: visual fill via keyboard then move in normal mode', () => {
    ...
    // Should be back in normal mode
    assert.equal(eng.mode.current, MODES.NORMAL, 'Should be in NORMAL mode after fill');

    // Check fill worked correctly (3x3 area from 2,2 to 4,4)
    assert.equal(eng.grid.cells[2][2], 5);
    assert.equal(eng.grid.cells[3][3], 5);
    assert.equal(eng.grid.cells[4][4], 5);

    // Check outside area is NOT filled
    assert.equal(eng.grid.cells[0][0], null);
    assert.equal(eng.grid.cells[5][5], null);

    // User moves cursor with hjkl in normal mode - should NOT paint
    eng.cursor.setPosition(0, 0);
    press(eng, 'l'); // move right
    assert.equal(eng.grid.cells[0][0], null, 'Moving in normal mode should not paint');

    press(eng, 'j'); // move down
    assert.equal(eng.grid.cells[1][0], null, 'Moving in normal mode should not paint');

    press(eng, 'l'); // move right again
    assert.equal(eng.grid.cells[1][1], null, 'Moving in normal mode should not paint');
  });
  ...
  it('user scenario: visual flood fill (lowercase f) should only fill selected area', () => {
    ...
    // Should be back in normal mode
    assert.equal(eng.mode.current, MODES.NORMAL, 'Should be in NORMAL mode after flood fill');

    // Flood fill should respect selection bounds
    let filledCount = 0;
    for (let y = 0; y < eng.grid.height; y++) {
      for (let x = 0; x < eng.grid.width; x++) {
        if (eng.grid.cells[y][x] === 5) filledCount++;
      }
    }
    ...
    assert.equal(eng.grid.cells[2][2], 5); // inside selection
    assert.equal(eng.grid.cells[4][4], 5); // inside selection
    assert.equal(eng.grid.cells[0][0], null); // outside selection
    assert.equal(eng.grid.cells[5][5], null); // outside selection
  });
  ...
  it('paste works in normal mode with p key', () => {
    ...
    eng.cursor.setPosition(5, 5);
    press(eng, 'p');

    // Should still be in normal mode
    assert.equal(eng.mode.current, MODES.NORMAL, 'Should remain in normal mode after paste');

    // Check paste worked
    assert.equal(eng.grid.cells[5][5], 3);
    assert.equal(eng.grid.cells[6][5], 3);
    assert.equal(eng.grid.cells[5][6], 3);
    assert.equal(eng.grid.cells[6][6], 3);
  });
  ...
  it('x key cuts cell (delete and yank to clipboard)', () => {
    ...
    // Paint a cell
    eng.setColorIndex(7);
    eng.cursor.setPosition(2, 3);
    eng.paint();
    assert.equal(eng.grid.cells[3][2], 7);

    // Cut it with x
    press(eng, 'x');

    // Should be erased
    assert.equal(eng.grid.cells[3][2], null, 'Cell should be erased after cut');
    ...
    // Should be pasted
    assert.equal(eng.grid.cells[5][5], 7, 'Cut cell should be in clipboard and pasteable');
    assert.equal(eng.mode.current, MODES.NORMAL);
  });
});
