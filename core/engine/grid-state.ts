...
export class GridState {
  ...
  private _cells: Array<Array<string | null>>;

  constructor(width: number, height: number) {
    this._width = clamp(width | 0, 1, 256);
    this._height = clamp(height | 0, 1, 256);
    this._cells = Array.from({ length: this._height }, () => Array<string | null>(this._width).fill(null));
  }
  ...
  writeCell(x: number, y: number, value: string | null | undefined) {
  ...
  load(snapshot: { width: number; height: number; grid: Array<Array<string | null>> }) {
